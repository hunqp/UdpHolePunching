...
int main()
{
    ...
    mosquitto_subscribe(mosq, NULL, CLIENTTOPIC, 0);
    ...
    while (1)
    {
        ...
        memset(&recvbuffer, 0, MAXLINE);
        n = recvfrom(sockfd, (char *)recvbuffer, MAXLINE, MSG_WAITALL, (struct sockaddr *)&cliaddr, &len);
        buffer[n] = '\0';
        if (n < 1)
        {
        ...
        printf("[%d] Recevied from %s:%d : %s\n", n, clientip, clientport, recvbuffer);
